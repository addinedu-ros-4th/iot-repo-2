...
class iotComputer(QMainWindow, from_class):
    ...
    def __init__(self) :
        ...
        self.pySerial = sri.Serial(port="/dev/ttyACM1", baudrate=9600)# serial
        ...
        self.planList = ""
        self.mealCountList = ""
        self.totalHeight = 20 # 수조 전체 높이 설정 
    ...
    def updateInput(self) : # 아두이노신호 받는 함수 (반복됨)
        if self.pySerial.in_waiting != 0:
            ...
            try:
                ...
                self.temperature = decodedDict["waterTemperature"] ### 이후 아두이노 input으로 대체
            except SyntaxError:
                pass
            ...
            self.waterQualityLabel.setText(str(self.waterQulity) + "mg")
    ...
    def showStatusOfFishbowl(self) :
        ...
        if self.waterQulity > 300 : # 센서도착시 확인후 변경
            text += " 물 교체 필요"
            self.qualityStateLabel.setText("부적합")
        else:
            self.qualityStateLabel.setText("적합")
    ...
    def drawChart(self, x, y):
        ...
        axis = pg.AxisItem(orientation='left')  # 그래프에 y축 추가하기 
        self.levelGraph.addItem(axis)
